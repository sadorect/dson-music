...
<body class="brand-sitewide font-sans antialiased bg-white">
        ...
        <div class="w-full md:w-[500px]  px-6 py-8">
            <div class="flex  justify-center gap-4 flex-col items-center">
                <a href="/" class="flex items-center">
                    ...
                    @else
                        <span class="text-xl font-bold text-black">{{ setting('site_name') }}</span>
                    @endif
                </a>
            ...
            <div class="w-full rounded-xl p-8 bg-white border border-black/10 shadow-sm">
                {{ $slot }}
            </div>
            
            <div class="mt-8 text-center text-sm text-black/70">
                <p>&copy; {{ date('Y') }} {{ setting('site_name') }}. All rights reserved.</p>
                <div class="mt-2 flex justify-center space-x-4">
                    @if(setting('social_links.facebook'))
                        <a href="{{ setting('social_links.facebook') }}" target="_blank" class="text-black/70 hover:text-orange-600">
                    ...
                    @if(setting('social_links.twitter'))
                        <a href="{{ setting('social_links.twitter') }}" target="_blank" class="text-black/70 hover:text-orange-600">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    @if(setting('social_links.instagram'))
                        <a href="{{ setting('social_links.instagram') }}" target="_blank" class="text-black/70 hover:text-orange-600">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    @if(setting('social_links.tiktok'))
                        <a href="{{ setting('social_links.tiktok') }}" target="_blank" class="text-black/70 hover:text-orange-600">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
