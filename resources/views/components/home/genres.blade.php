...
<div class="container mx-auto px-4">
    <h2 class="text-2xl md:text-3xl font-bold mb-6 text-white">Explore Genres</h2>
    ...
    @else
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4">
            @foreach($genres as $genre)
                @if(!empty($genre))
                    <a href="#" class="block bg-gradient-to-r from-primary-color to-primary-color/80 text-white rounded-full p-4 text-center hover:from-primary-color/80 hover:to-primary-color/80 transition-all">
