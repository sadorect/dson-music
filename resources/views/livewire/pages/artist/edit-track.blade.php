...
new #[Layout('layouts.glass-app')] class extends Component
{
    ...
    protected function rules(): array
    {
        $rules = [
            ...
            'audioFile' => ['nullable', 'file', new ValidAudioUpload(), 'max:' . UploadLimits::audioKb()],
    ...
    public function save(): void
    {
        ...
        try {
            ...
            if ($this->audioFile) {
                ...
                $this->addStagedMedia($this->track, $this->audioFile, 'audio', $audioFileName, 'audioFile');
            }
            ...
            $this->addError('save', "We couldn't save your track changes right now. Please try again.");
        ...
        <div class="glass-card rounded-2xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div>
                    ...
                    <select wire:model="mood" id="mood"
                        class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-red-400 focus:ring-red-400 text-sm">
                        <option value="">No mood</option>
                        @foreach($moods as $m)
                            <option value="{{ $m['slug'] }}" @selected($mood === $m['slug'])>{{ $m['label'] }}</option>
                        @endforeach
                    </select>
                    @if(!$mood && $this->suggestedMoodLabel())
                        <p class="mt-1 text-xs text-gray-400">Suggested from genre: {{ $this->suggestedMoodLabel() }}</p>
                    @else
                        <p class="mt-1 text-xs text-gray-400">Used for vibe-led browse and recommendation placement.</p>
                    @endif
        ...
        <div class="glass-card rounded-2xl p-6 space-y-3">
            ...
            <div class="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Upload a new image to replace the current cover - JPG, PNG, WEBP, up to {{ UploadLimits::formatKilobytes(UploadLimits::imageKb()) }}</p>
